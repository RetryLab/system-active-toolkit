...
Tcpx.prototype.readStat = function (callback) {
	...
	async.parallel({
		...
		if (err) {
			...
			self.fullStat = _.extend(result.snmp, result.netstat)
			self.fullStat.tcpnconnest = self.fullStat.activeopen + self.fullStat.passiveopen;


			callback(null, self.fullStat);
		}
	})
}
